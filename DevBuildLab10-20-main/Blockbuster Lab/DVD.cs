                ...
                try
                {
                    int userSelect = int.Parse(Console.ReadLine());
                        ...
                        if (userSelect > 0 && userSelect <= SceneList.Count)
                        {
                            ...
                            Console.WriteLine($"Playing {Title} from scene {userSelect}: {SceneList[userSelect - 1]}.  Runtime starting at {currentTime} mins\n");//needed to modify the scene list so that it indexes correctly
